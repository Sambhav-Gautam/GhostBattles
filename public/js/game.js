...
(() => {
    ...
    function initScene() {
        ...
        scene.fog = new THREE.FogExp2(0x050510, 0.035); // Spooky fog
    ...
    function addKillFeedItem(killer, victim) {
        ...
        item.innerHTML = `<span class="killer">${esc(killer)}</span> <span class="kill-icon">${Assets.getIcon('SKULL')}</span> <span class="victim">${esc(victim)}</span>`;
    ...
    function showEffect(text, ms) {
        ...
        const effectIcon = document.getElementById('effect-icon-container');
        if (text.includes('Speed')) Assets.inject('effect-icon-container', 'LIGHTNING');
        else if (text.includes('Damage') || text.includes('Fire')) Assets.inject('effect-icon-container', 'FIRE');
        else Assets.inject('effect-icon-container', 'HEART');
    ...
    function startGame() {
        ...
        InputHandler.onAbility(() => {
            ...
            const now = Date.now();
            const cd = getAbilityCooldown(localGhostType);
            if (now - lastAbilityUseTime < cd) {
                showEffect('Ability Cooldown!', 500);
                return;
            }

            socket.emit('use-ability');
            // We'll update lastAbilityUseTime on server confirmation 'ability-used'
            // But for responsiveness, we can optimistically set it here too?
            // Better to wait for server or use optimistic UI.
            lastAbilityUseTime = now;
            startAbilityCooldownUI(cd);
        });
    }
    ...
    function getAbilityCooldown(type) {
        return { wraith: 4000, phantom: 10000, shade: 8000, specter: 15000 }[type] || 5000;
    }

    function startAbilityCooldownUI(duration) {
        const hud = document.getElementById('game-hud');
        let cdEl = document.getElementById('ability-cooldown');
        if (!cdEl) {
            cdEl = document.createElement('div');
            cdEl.id = 'ability-cooldown';
            cdEl.style.cssText = `
                position: absolute;
                bottom: 20px;
                right: 20px;
            ...
            cdEl.appendChild(label);
            hud.appendChild(cdEl);
        }
        ...
        const interval = setInterval(() => {
            ...
            if (remaining <= 0) {
                ...
                cdEl.style.borderColor = '#4ade80'; // Green on ready
                cdEl.firstChild.textContent = 'READY';
                setTimeout(() => {
                    cdEl.style.borderColor = '#a855f7';
                    cdEl.firstChild.textContent = 'ABILITY';
                }, 1000);
            } else {
                cdEl.style.borderColor = '#ef4444'; // Red on cd
                cdEl.firstChild.textContent = (remaining / 1000).toFixed(1) + 's';
            }
        }, 100);
    }
}
...
function updateWeaponHUD() {
    ...
    Assets.inject('weapon-icon-container', config.icon);
